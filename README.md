...
| Generation | GPT-4o-mini | Handles planning, grading, reformulation, and synthesis. Temperature zero everywhere. |
...
A cross encoder reranker does. It takes the query and a candidate chunk as a single input and produces a relevance score with full cross attention between them. I send all 20 RRF candidates through the reranker and drop anything below the relevance threshold. Usually 3 to 5 chunks survive. This is the step that turns a broad candidate pool into a tight, genuinely relevant context window.
